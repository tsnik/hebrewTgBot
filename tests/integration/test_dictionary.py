...
async def test_delete_word_from_dictionary(memory_db):
    ...
    with UnitOfWork() as uow:
        ...
        word_id_to_delete = uow.words.create_cached_word(
            hebrew='למחוק', normalized_hebrew='למחוק', transcription='limkhok',
            is_verb=False, root=None, binyan=None,
            translations=[{'translation_text': 'to delete', 'is_primary': True}], conjugations=[]
        )
        word_id_to_keep = uow.words.create_cached_word(
            hebrew='לשמור', normalized_hebrew='לשמור', transcription='lishmor',
            is_verb=False, root=None, binyan=None,
            translations=[{'translation_text': 'to keep', 'is_primary': True}], conjugations=[]
        )
    ...
    with UnitOfWork() as uow:
        ...
        for i in range(DICT_WORDS_PER_PAGE + 1):
            word_id = uow.words.create_cached_word(
                hebrew=f'מילה{i}', normalized_hebrew=f'מילה{i}', transcription=f'mila{i}',
                is_verb=False, root=None, binyan=None,
                translations=[{'translation_text': f'word{i}', 'is_primary': True}], conjugations=[]
            )
