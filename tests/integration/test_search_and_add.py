...
async def test_search_and_add_flow_e2e(monkeypatch):
    ...
    # Создаем мок AsyncClient
    mock_async_client = AsyncMock()
    mock_response = Mock()
    mock_response.text = html_content
    mock_response.status_code = 200
    # URL должен быть таким, как будто мы перешли на страницу слова
    mock_response.url = "https://www.pealim.com/dict/9999-bdika/"
    mock_async_client.get.return_value = mock_response
    
    # "Подменяем" httpx.AsyncClient в модуле парсера с помощью monkeypatch
    monkeypatch.setattr("app.services.parser.httpx.AsyncClient", lambda **kwargs: mock_async_client)

    # --- 3. Вызываем обработчик, который запускает всю цепочку ---
    await handle_text_message(mock_update, mock_context)

    # --- 4. Проверяем состояние БД после реального парсинга и сохранения ---
    ...
    # Важно: используем новый mock_update для callback_query
    ...
    # Передаем исходное сообщение, чтобы его можно было отредактировать
