...
def get_test_connection(memory_db_uri: str) -> sqlite3.Connection:
    ...
    connection = sqlite3.connect(memory_db_uri, uri=True, detect_types=sqlite3.PARSE_DECLTYPES)
...
def test_word_repository(memory_db):
    ...
    with connection:
        word_id = repo.create_cached_word(
            hebrew='לִכְתּוֹב', normalized_hebrew='לכתוב', transcription='likhtov',
            is_verb=True, root='כ-ת-ב', binyan='פעל',
            translations=translations, conjugations=conjugations
        )
...
def test_user_dictionary_repository(memory_db):
    ...
    with connection:
        ...
        word_id = word_repo.create_cached_word(
            hebrew='שֻׁלְחָן', normalized_hebrew='שולחן', transcription='shulchan',
            is_verb=False, root=None, binyan=None,
            translations=translations, conjugations=[]
        )
...
def test_word_repository_transaction_rollback(memory_db):
    ...
    with pytest.raises(TypeError):
        with connection: 
            repo.create_cached_word(
                hebrew='לְהִכָּשֵׁל', normalized_hebrew='להכשל', transcription='lehikashel',
                is_verb=True, root='כ-ש-ל', binyan='נפעל',
                translations=translations, conjugations=invalid_conjugations
            )
    ...
    assert found_word is None, "Word should not have been created due to transaction rollback."
